...
def main():
        ...
        for i in range(len(fermi_time_array)):
                ...
                submit_dag_file.write("VARS TS" + str(i) + ''' line="''' + str(fermi_time_array[i]) + '''" dir=" ''' + os.path.join(output_path, str(time_list[int(i/1440)*1440]).split('T')[0], str(time_list[i]).split('T')[1][0:2]) + '''" output=" ''' + os.path.join(output_path, str(time_list[int(i/1440)*1440]).split('T')[0], "output_TS",str(time_list[i]).split('T')[1].replace(":", "/")) + '''"\n''')
        for i in range(len(fermi_time_array)):
                submit_dag_file.write("PARENT")
                for j in range(int(len(time_list)/60)):
                        submit_dag_file.write(" A" + str(int(j/24)) + " B"+ str(j))	
