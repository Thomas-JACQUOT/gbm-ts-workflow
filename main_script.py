...
def main():
        ...
        submit_download.write("Arguments  = --time $(time) --format datetime --output " + output_path + "\n")
        submit_download.write("input      = /dev/null\n")
        submit_download.write("Log        =" +  error_submit_download  + "/download_condor_poshist.log\n")
        submit_download.write("error      =" + error_submit_download + "/download_condor_poshist_$(time).err\n")
        ...
        submit_download.write("error      =" + error_submit_download + "/download_condor_tte_$(time).err\n")
        submit_download.write("output     = /dev/null\n")
        submit_download.write("notification = never\n")
        submit_download.write("getEnv     = True\n")
        submit_download.write("accounting_group = ligo.prod.o4.cbc.grb.gbm_subthreshold\n")
        ...
        for i in range(int(len(time_list)/1440)):
                ...
                submit_dag_file.write("VARS A" + str(i) + ''' time="''' + str(time_list[i*1440]) + '''"\n''')
        ...
        for i in range(int(len(time_list)/60)):
                ...
                submit_dag_file.write("VARS B" + str(i) + ''' time="''' + str(time_list[i*60]) + '''"\n''')
        ...
        for i in range(len(fermi_time_array)):
                ...
                for j in range(int(len(time_list)/1440)):
                        submit_dag_file.write(" A" + str(int(j)) + " B"+ str(j))	
